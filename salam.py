...
def validation():
    ...
    if not birth.isdigit():
        messagebox.showerror("Error","Birthyear should be a number")
        return False
    return True
...
def create():
    ...
    workbook = op.loadworkbook('excelDB.xlsx')
    sheet = workbook.active
...
def update():
    ...
    for rows in sheet.iter_rows(min_row=2):
        if str(record_id) == str(rows[0].vlue):
            rows[1].value = last
            rows[2].value = first
            rows[3].value = middle
            rows[4].value = birth
...
def delete():
    ...
    confirm = messagebox.askyesnocancel("Confirm","Are you sure you want to delete this fucking record")
    if not confirm:
        return
...
tree.bind("<<TreeviewSelected>>",auto_populate)
